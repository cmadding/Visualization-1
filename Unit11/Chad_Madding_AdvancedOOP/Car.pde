class Car extends Obstacle {
  ...
  Car (float _x, float _y, float _d) {
    x = _x;
    y = _y;
    d = _d;
  }
  ...
  void act() {
    ...
    //loop to check every bullet to see if it hits the Car 
    int i = 0;
    ...
    while ( zl < zombieList.size()) {
      ...
      if ( dist(z.x, z.y, x, y) <  z.d/2 + d/2) { //checking the distance between the Bullet (b.x, b.y) and the tree (x, y) sum of radi
        hit = true;
        z.alive = true;
        z.x = z.x - bounce;
        z.y = z.y - bounce;
